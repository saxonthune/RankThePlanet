...
The surface has three regions. The **topBar** names the Entry by its Location and carries the Entry-level affordances — *Back* to the owning Collection and *Remove the Collection Entry* (`Collection.removeEntry`, which drops the Entry without deleting the Location). The **location** region shows the Location — display name, coordinates, cached provider metadata — and its two leaf handoffs: *Open in an external map app* (`Location.openExternally`) and *Refresh the Location's cached metadata* (`Location.refresh`). The **review** region renders the Review as the full template field set, with *Edit the Review* (`Review.edit`) opening the `ReviewForm`.

A Review is never absent — an Entry always carries one ([[03-concepts]], doc01.03 §3). The state the review region renders is **reviewed** vs **unreviewed**: an unreviewed Entry shows every template field unset and prompts toward *Edit the Review*; a reviewed Entry shows whatever the user has saved, however sparse. There is no "incomplete" state in between — a one-field Review is a finished Review.
