...
The provider list is the **whole** provider catalogue the app supports — not just the configured ones. An unconfigured BYOK provider still has its row, so the path to "I want to start using Google Places" is one tap on the row, not a hidden "add provider" affordance ([[04-location-providers]], doc03.04). Nothing here names a Composable or a UiState; that binding is not part of the interaction layer (doc02.02.00).
