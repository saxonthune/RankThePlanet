...
The **collectionsButton** region is an extended floating action button anchored to the bottom-end corner over the map, holding *Browse Collections* (`TAP_COLLECTIONS_BUTTON` → `CollectionList`).
...
- **collectionsButton** (*Browse Collections*) — suppressed; the user cannot bounce into the Collection list mid-flow.
