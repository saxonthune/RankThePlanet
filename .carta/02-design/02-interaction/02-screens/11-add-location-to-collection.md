...
The surface has no top bar. Two regions. The **candidate** region reflects the Location being added — display name when the user adopted a nearby candidate, raw coordinates otherwise — read-only; the user confirms the candidate by picking a Collection, not by editing it here. The **collections** region is a vertical list of the user's Collections, where tapping one fires `PICK_COLLECTION` (`Location.addToCollection`), commits the Entry, and dismisses the sheet back to [[07-map-overview]] (`MapOverview`). The host carries the new Entry as `pending-review-context` and renders a Material 3 Snackbar offering to jump into [[06-review-form]] (`ReviewForm`) — landing on the map instead of the form preserves the user's map context after a successful add. The list also holds *New Collection* (`TAP_NEW_COLLECTION` → `CollectionEditor`, `Collection.create`) as the first row, so creating a new Collection mid-flow is one tap away. A *Back* affordance in the same region steps one surface back to `LocationDraftSheet` — the draft sheet re-presents with the same candidate intact.
