...
The affordance inventory for the `CollectionEntryDetail` surface — one Collection Entry, the `(Location, Review)` pair ([[03-concepts]], doc01.03). **The source of truth is the rhidoc sidecar `04-collection-entry-detail.inventory.json`** ([[00-index]], doc02.02.02.00 explains the shape); this `.md` is a lean companion.
