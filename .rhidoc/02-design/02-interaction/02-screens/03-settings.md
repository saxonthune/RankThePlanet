...
The affordance inventory for the `Settings` surface — where the user configures providers, sync target, and BYOK keys. **The source of truth is the rhidoc sidecar `03-settings.inventory.json`** ([[00-index]], doc02.02.02.00 explains the shape); this `.md` is a lean companion.
