...
The affordance inventory for the `LicenseViewer` surface — the in-app view of the bundled `LICENSE` reachable from `About` ([[15-about]], doc02.02.02.15). **The source of truth is the carta sidecar `16-license-viewer.inventory.json`** ([[00-index]], doc02.02.02.00 explains the shape); this `.md` is a lean companion.
