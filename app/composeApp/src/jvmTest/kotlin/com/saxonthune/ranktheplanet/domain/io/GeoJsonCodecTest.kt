...
class GeoJsonCodecTest {
    ...
    fun roundTripPreservesCollectionMetadata() {
        ...
        val json = GeoJsonCodec.encode(testCollection, testTemplate, entries)
        val result = GeoJsonCodec.decode(json)
    ...
    fun roundTripAddressNullVsPresent() {
        ...
        val json = GeoJsonCodec.encode(testCollection, null, listOf(withAddr, withoutAddr))
        val result = GeoJsonCodec.decode(json)
        assertTrue(result.isSuccess)
    ...
    fun decodeInvalidJsonReturnsFailure() {
        val result = GeoJsonCodec.decode("not json at all {{{")
    ...
    @Test
    fun decodeMinimalForeignFeatureCollection() {
        val json = """
            {
              "type": "FeatureCollection",
              "features": [{
        ...
        assertEquals(48.8566, entry.location.coordinates.lat)
        assertEquals(2.3522, entry.location.coordinates.lng)
    }
}
