...
State machines, JSON Schemas, and other machine-readable artifacts live as **sidecars** in the same rhidoc bundle as their host `.md` file (`02-navigation.md` + `02-navigation.json`). Rhidoc's structural ops (move, rename, punch, flatten) treat the bundle as a unit. Never create a `.statemachines/` or `.schemas/` directory.
...
- **Rhidoc is the substrate.** Doc structure operations always go through `rhidoc` — never hand-edit numbering or move files manually. See `.rhidoc/AGENTS.md` for the AI navigation patterns.
