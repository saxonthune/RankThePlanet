#!/usr/bin/env node
// verify.mjs — rhidoc affordance-inventory verification harness
//
// Walks .rhidoc/ for *.md docs declaring a `verify` frontmatter field and runs
...
//
// CLI: node .rhidoc/verify.mjs [path]
//   no arg  — scan all of .rhidoc/
...
// Walk from RHIDOC_ROOT matching NN-* directories for each non-final segment,
...
// *.inventory.json under RHIDOC_ROOT. For each transition in the chart:
//
    ...
    for (const entry of readdirSync(dir)) {
      ...
      if (statSync(full).isDirectory()) walk(full);
        ...
        try {
          ...
          // Malformed JSON surfaces elsewhere (rhidoc verify, JSON parse on demand).
        }
      }
    }
  }
...
function verifyGuardCoverage(statechartPath) {
  const statechart = JSON.parse(readFileSync(statechartPath, 'utf8'));
  const states = statechart.states || {};
  const inventories = loadAllInventories(RHIDOC_ROOT);
  const issues = [];
...
// *.inventory.json under RHIDOC_ROOT.
...
function main() {
  ...
  const root = arg ? resolve(arg) : RHIDOC_ROOT;
  ...
  for (const mdPath of mdFiles) {
    ...
    const rel = relative(RHIDOC_ROOT, mdPath);
    console.log(`\n${rel}`);
