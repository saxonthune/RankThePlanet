...
The affordance inventory for the `DebugSettings` surface — a developer pocket reached from Settings's *Debug* affordance ([[03-settings]], doc02.02.02.03). **The source of truth is the rhidoc sidecar `14-debug-settings.inventory.json`** ([[00-index]], doc02.02.02.00 explains the shape); this `.md` is a lean companion.
