...
The surface is full-screen (`meta.modality: fullScreen`) and the **host** for three sheet surfaces — `LocationDraft`, `LocationDetail`, `EntryDrawer` — declared in `meta.hostsSheets`. Sheets render over the map without unmounting it; their state lives in this surface's UiState ([[04-surface-composition-rules]], doc02.04). Four regions are always present — **map**, **menuButton**, **search**, **bottomBar** — plus the **menuDrawer**, which is present but closed by default, and a sixth, **statusBar**, which appears only in add-to-collection mode (below).
