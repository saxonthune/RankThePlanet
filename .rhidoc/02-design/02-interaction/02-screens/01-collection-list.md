...
The affordance inventory for the `CollectionList` surface — where the user browses Collections and decides which ones the map renders. **The source of truth is the rhidoc sidecar `01-collection-list.inventory.json`** ([[00-index]], doc02.02.02.00 explains the shape); this `.md` is a lean companion.
