...
The affordance inventory for the `LocationDraftSheet` surface — where a dropped pin is inspected before it becomes a committed Location. Search-picked provider candidates skip this surface entirely (they already carry a fully resolved Location): in browse mode they land on `LocationSheet` for the peek; in add-to-collection mode they land directly on `AddLocationToCollection`. **The source of truth is the carta sidecar `05-location-draft.inventory.json`** ([[00-index]], doc02.02.02.00 explains the shape); this `.md` is a lean companion.
