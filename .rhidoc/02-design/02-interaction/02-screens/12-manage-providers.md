...
The affordance inventory for the `ManageProviders` surface — the provider catalogue reached from Settings's *Manage providers* affordance ([[03-settings]], doc02.02.02.03). **The source of truth is the rhidoc sidecar `12-manage-providers.inventory.json`** ([[00-index]], doc02.02.02.00 explains the shape); this `.md` is a lean companion.
