...
The surface has three regions. The **topBar** carries *Cancel* (`CLOSE`), which discards uncommitted changes, and *Finish editing* (`DONE`), which commits and continues into `CollectionDetail`. When the editor is reached with `return-to-draft-context` set — the entry from [[11-add-location-to-collection]] (doc02.02.02.11) `TAP_NEW_COLLECTION`, where an in-flight candidate Location is waiting in the `MapOverview` host's UiState — the same Save affordance fires `DONE_AS_DRAFT_RETURN` instead. The host pops back to `MapOverview`, commits the candidate against the just-created Collection (`Location.addToCollection`), and surfaces the same post-add Snackbar as `AddLocationToCollection`'s `PICK_COLLECTION`. The two events name the same button — the schema permits one transition per event, so the alternate target gets its own event id. The **metadata** region edits the Collection's `name`, `description`, and `appearance` (`Collection.edit`) — this is the only place that metadata is editable; `CollectionDetail` shows it read-only ([[02-collection-detail]], doc02.02.02.02). The **template** region authors the Review template: *Adopt a built-in template* (`Review.useBuiltIn`) seeds the field set, *Add a template field* and the per-field *Edit, reorder, or remove* affordance shape it (`Review.editTemplate`).
...
A field's `name` is its stable machine key — immutable once defined ([[03-concepts]], doc01.03 §3). The per-field editor lets the user change `label`, `type`, `config`, and `required` on an existing field, and lets `name` be set only on a newly added field. Reordering and removal stay available on every field.
